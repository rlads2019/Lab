---
title: "Strings & Regular Expressions (W8)"
date: '2019-10-31'
output:
  html_document:
    number_sections: no
...
1. 找出所有**記者姓名**
    ...
    ```{r results='asis', echo=F}
    regex_view(news, '【\\w{3}╱\\w+報導】', showInput = F)
    ```

1. 找出所有**百分比** (e.g. `20%`, `17.5%`)

    ```{r results='asis', echo=F}
    regex_view(news, '[\\d.]+%', showInput = F)
    ```

### Backreference

  ```{r results='asis', echo=F}
  AABB <- readLines('corpus/AABB.txt')
  ABCA <- readLines('corpus/ABCA.txt')
...
### Your Turn
  ...
  ```{r results='asis', echo=F}
  regex_view(readLines('corpus/construction.txt'),
...
- 在 R 裡面，Regular Expression 是以**字串**的資料類型 (i.e. `character`) 去表徵，因此有一點需特別注意：
  ...
  若 Regular Expression 裡面出現**反斜線**，在將此 Regex 寫進 R 的函數時，需要在**每一個反斜線之前再加上一個反斜線**:
...
- `str_match()` 可從文字裡抓出符合 RegEx 的字串。`str_match()` 只會抓出 `str` 每個元素裡**第一個符合的字串**，並且以 `matrix` 的資料結構回傳：
    ...
    ```{r}
    str <- c("", "One two three. Four five six.", "Seven eight nine.")
    str_match(str, "(\\w+) (\\w+) (\\w+)")
    ```
...
- 若要抓出**所有符合的字串**，需使用 `str_match_all()`[^all]。`str_match_all()` 會抓出 `str` 每個元素裡**所有符合的字串**，並以 `list` 的資料結構回傳：
    ...
    ```{r}
    str <- c("", "One two three. Four five six.", "Seven eight nine.")
    str_match_all(str, "(\\w+) (\\w+) (\\w+)")
    ```

    - 回傳的 list 中，list 的每個元素依序對應到 `str` 的每個元素。因此，回傳的 list 長度為 2 (與 `str` 相同)
...
<button id="show-regex" class="button-show">show</button>

```{js echo=FALSE}
window.onload = () => {
  var button = document.querySelector('button#show-regex');
  button.onclick = function() {
    
    if (button.className == 'button-show') {
        document.querySelectorAll('.hide').forEach(ele => {
                ele.className = 'regex-input show';
        })
      
        button.className = 'button-hide'
        button.innerHTML = 'hide'
      } else if (button.className == 'button-hide') {
        document.querySelectorAll('.show').forEach(ele => {
                ele.className = 'regex-input hide';
        })
