...
- `httr::GET()` 會將 HTTP request 以及接收到的 HTTP response 轉換成 R 的物件。例如，`req[["url"]]` 可以取得 HTTP request 目的地 URL，`req[["status_code"]]` 則可以看到伺服器回傳 (HTTP response) 的訊息 (200 代表 request 成功，4** 通常代表失敗)：
...
HTTP request 有許多「不同」的種類，稱為 [request methods](https://developer.mozilla.org/en-US/docs/Web/HTTP/Methods)。最常見的一種即是 `GET method`，例如在網頁輸入 URL 或是剛剛使用的 `httr::GET()`， 都是在向伺服器發出 `GET` request。`GET` request 的目的是向伺服器**取得**資料。另一種常見的 request method 是 `POST`。`POST` request 的目的是向伺服器**提交**資料，常見的例子是「帳號登入」與「填寫表單」。關於 HTTP request methods 可見 <https://developer.mozilla.org/en-US/docs/Web/HTTP/Methods>。
...
- JSON 所記錄的資料，結構上與 R 的 list 非常接近。下圖左邊的 R 程式碼所建立出的 list，其表徵的資料結構，等價於右方的 JSON:
    ![](json_v_list.png)
...
- 在 JSON 格式裡，`{}` 之內的 key-value pair 並無次序關係。若要需要表徵先後次序，需使用 array: `[]`。在 `[]` 中，可以放入任意數量以及各個類別的資料 (彼此以 `,` 分隔)，例如：
