...
- Markdown 輸出 HTML
    - 不清楚在 RStudio 寫 Markdown 的樣子和上傳到 GitHub 後的樣子不太一樣
    - 原因是 GitHub 會將上傳的 Markdown 轉換成 HTML，而瀏覽器讀到 HTML 的時候，會將它轉換成你所看到的網頁
    - 這也是為什麼 Markdown 可以與 HTML 混用的關係，因為 Markdown 它最終會被轉換成 HTML

(next slide)

- 大家不見得有發覺，但 (我覺得) Markdown 比想像中的難，原因有兩個
    1. 有些語法隱晦
    2. 第二個原因則是來自我們對於排版的控制慾
        - 我們都很習慣使用 Word 來撰寫和排版我們的文章
        - 久而久之，讓我們對文章的樣式形成一些個人偏好
           - 例如，有些人就習慣新的段落開頭一定要縮排、標題一定要粗體
        - 我們自然而然地就將這種對於排版的控制慾帶入 Markdown 裡面
        - 但 Markdown 它是一種非常簡約的標記式語言，
            - 所以它只定義了文字的「結構」，像是「標題」、「段落」、「清單」和「圖片」等等
            - 但是它並未去定義這些「結構」**該長什麼樣子**
        - 所以，知道 Markdown 可以與 HTML 混用的人，很容易去使用 HTML 的語法當成一個修飾文件「樣式」的工具，例如，段落開頭加入 HTML 的空白字元當作縮排效果
        - 隨著加入越多的「樣式」，以 Markdown 撰寫的文件就不再是 Markdown，反而比較接近 HTML 文件，這會讓你撰寫 Markdown 文件的過程變得非常痛苦
        - 所以這邊的困難點在於，Markdown 它無法滿足我們排版的控制慾，如果要滿足這種欲望，必須先學會 HTML 和 CSS，才能在 Markdown 輸出成 HTML 之後，透過 CSS 去控制這份文件的樣式。
</div>
...
2. 「資料分析」觀點：
    - 結構化資料：Excel 2 維表格式的資料、R 稱為 data frame --> 介紹套件處理 2 維資料
    - 資料視覺化：ggplot2
    - 目標：EDA、資料裡面游泳，譬如可以輕鬆地將資料整理成想要的樣子，這樣才能進一步地去跑一些統計和繪圖，幫助你了解資料大致的樣貌,
    - 衡量標準：能從 google 表單匯出 `.csv` 檔後，不手動觸碰 `.csv` 下，完全使用 R 語言去幫助自己將資料整理成自己想要的樣子
3. 「文本分析」觀點：
    - 從「結構化」移往「非結構化」資料的處理
    - 字串處理
    - 中文文本處理
        - 斷詞
    - 程式與資料清理技巧，將非結構化的資料轉換成結構化資料
</div>
